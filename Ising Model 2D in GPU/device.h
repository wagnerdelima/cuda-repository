		...
		if (blockIdx.x == 0) { //Top
			...
			if (threadIdx.x == BLOCK_SIZE - 1) { //Right
				...
				sum_spins =  (S[2 * threadIdx.x + 2 - 2 * BLOCK_SIZE]
					+ S[2 * threadIdx.x] + S[2 * threadIdx.x + 1 + 2 * BLOCK_SIZE]
					...
					sum_spins = S[2 * threadIdx.x + 2] + S[2 * threadIdx.x] + S[2 * threadIdx.x + 1 + 2 * BLOCK_SIZE] + S[2 * threadIdx.x + 1 + N - 2 * BLOCK_SIZE];
			...
			if (threadIdx.x == 0) { //Left
				...
				sum_spins =  (S[2 * threadIdx.x + 4 * BLOCK_SIZE * blockIdx.x + 2 * BLOCK_SIZE + 1]
					...
					sum_spins =  (S[2 * threadIdx.x + 4 * BLOCK_SIZE * blockIdx.x + 2 * BLOCK_SIZE + 1]
						...
						+ S[2 * threadIdx.x] + S[2 * threadIdx.x + 4 * BLOCK_SIZE * blockIdx.x]);
